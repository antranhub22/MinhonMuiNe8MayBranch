...
Mi Nhon Hotel Mui Ne`
  ...
  app.post('/api/staff/requests', async (req, res) => {
    try {
      const { callId, roomNumber, guestName, orderId, content } = req.body;
      
      // Validate required fields
      if (!callId || !roomNumber || !content) {
        return res.status(400).json({ error: 'Missing required fields' });
      }
      
      // Create staff request
      const request = await storage.createStaffRequest({
        callId,
        roomNumber,
        guestName,
        orderId,
      ...
      // Add system message for request creation
      ...
      // Realtime: emit new request event
      const io = req.app.get('io');
      if (io) {
        io.to(roomNumber).emit('staff_request_new', { request });
      }
  ...
  app.get('/api/staff/requests', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to get staff requests' });
    }
  });
  ...
  app.get('/api/staff/requests/:id', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to get staff request' });
    }
  });
  ...
  app.patch('/api/staff/requests/:id/status', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to update staff request status' });
    }
  });
  ...
  app.post('/api/staff/requests/:id/messages', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to add staff message' });
    }
  });
